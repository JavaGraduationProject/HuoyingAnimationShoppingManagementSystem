    ...
    <div class="Content">
        ...
        <div class="ContentMain">
            <div class="renzhejieshao">
                <div class="UserAddFigureRefund">
                    <c:choose>
                        <c:when test="${UserRefundSubmit != null}">
                            <td>${UserRefundSubmit}</td>
                        </c:when>
                        <c:otherwise>
                            <form action="${pageContext.request.contextPath}/addFigureRefund" method="post" enctype="multipart/form-data">
                                ...
                                <div>
                                    ...
                                    <select id="refundType" name="refundType" required>
                                        ...
                                        <c:if test="${UserIsreceive eq 'true'}">
                                ...
                                <div>
                                    <label>内容：
                                        <em>*</em></label>
                                    <textarea name="refundReason" required></textarea>
                                </div>
                                <div>
                                    <div id="refundRsc">
                                        <label>图片：
                                            <em>*</em></label>
                                        <input id="refundRsc2" name="refundRsc" type="file" class="input-text" required="required">
                                    </div>
                                    <script>
                                        document.getElementById('refundType').onchange = function () {
                                            var select = document.getElementById('refundType');
                                            console.log(select.options[select.selectedIndex].innerText);//输出获取选中显示值
                                            console.log(select.value);//输出获取选中值
                                            if (select.value=="更改地址"){
                                                var UserRefundImg=document.getElementById('refundRsc');
                                                UserRefundImg.setAttribute("hidden",true);
                                                $('#refundRsc2').removeAttr('required');
                                            }
                                            else {
                                                var UserRefundImg=document.getElementById('refundRsc');
                                                UserRefundImg.removeAttribute("hidden");
                                                $('#refundRsc2').attr('required',"true");
                                            }
                                        }

                                        // var UserRefundTypeOptions=$("#refundType option:selected"); //获取选中的项
                                        // console.log(UserRefundTypeOptions.val());//拿到选中项的值option中value值
                                        // console.log(UserRefundTypeOptions.text());//拿到选中项的文本
                                        // if (UserRefundTypeOptions.val()=="更改地址"){
                                        //     document.getElementById('refundRsc').style.visibility='hidden';
                                        // }
                                        // else {
                                        //     document.getElementById('refundRsc').style.visibility='visible';
                                        // }

                                        // var UserRefundType=document.getElementById('refundType').value;
                                        // if (UserRefundType=="更改地址"){
                                        //     document.getElementById('refundRsc').style.visibility='hidden';
                                        //     // var UserRefundImg=document.getElementById('refundRsc');
                                        //     // UserRefundImg.setAttribute("hidden",true);
                                        // }
