    ...
    <div class="Content">
        ...
        <div class="ContentMain">
            <div class="UserApplyHyrzResource">
                <table>
                    <tr>
                        <td class="AdminShowAllResourceApply-HeadTd1">用户</td>
                        <td class="AdminShowAllResourceApply-HeadTd2">资源名称</td>
                        <td class="AdminShowAllResourceApply-HeadTd3">资源链接</td>
                        <td class="AdminShowAllResourceApply-HeadTd4">上传时间</td>
                        <td class="AdminShowAllResourceApply-HeadTd5" colspan="2">审核状态</td>
                    </tr>
                    <c:forEach var="ResourceApply" items="${requestScope.get('AdminShowAllResourceApply')}">
                        <tr>
                            <td>${ResourceApply.getUsername()}</td>
                            <td>${ResourceApply.getHyrzResourceName()}</td>
                            <td>${ResourceApply.getHyrzResourceLink()}</td>
                            <td>${ResourceApply.getApplyAddTime()}</td>
<%--                            <td>${ResourceApply.getPassStatus()}</td>--%>
                            <c:choose>
                                <c:when test="${ResourceApply.getPassStatus() eq '审核通过' or ResourceApply.getPassStatus() eq '审核不通过' }">
                                    <td colspan="2">已处理（${ResourceApply.getPassStatus()}）</td>
                                </c:when>
                                <c:otherwise>
                                    <td class=""><a href="${pageContext.request.contextPath}/AdminPassApplyResource/${ResourceApply.getResourceApplyAddID()}">审核通过</a></td>
