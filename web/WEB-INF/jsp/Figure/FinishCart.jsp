    ...
    <div class="Content">
        ...
        <div class="ContentMain">
            <div class="renzhejieshao">
                <div>
                    <h2 class="FinishCartH2Title">确认订单信息</h2>
                </div>
                <table class="FinishCart-table">
                    <tr><th>手办</th><th>手办价格</th><th>数量</th><th>总价</th></tr>
                    <c:forEach items="${cart}" var="item">
                        <input type="hidden" name="cid" value="${item.figure.figureId}" />
                        <tr>
                            <td class="td1">
                                <img src="/statics/images/figure/${item.figure.figureImage}" height="75" width="50" />
                                <div class="product-name">${item.figure.figureName}</div>
                            </td>
                            <td class="td2">
                                <p class="product-price">￥：${item.figure.figurePrice}</p>
                            </td>
                            <td class="td3">
                                <p class="product-price2">${item.quantity}</p>
                            </td>
                            <td class="td4">
                                <div class="product-price3">
                                    ￥：<fmt:formatNumber type="number" value="${item.quantity*item.figure.figurePrice}" pattern="0.00"/>
                                </div>
                            </td>
                        </tr>
                    </c:forEach>
...
<%--                <div>--%>
<%--                    <span class="title-text">运送方式：</span>--%>
<%--                    <label class="title-text">普通配送</label>--%>
<%--                    <span class="title-text">快递 免邮</span>--%>
<%--                </div>--%>
                <form class="FinishCart-form" action="${pageContext.request.contextPath}/toOrderPay">
                    <input type="hidden" name="userid" value="${user.userid}">
                    <table>
                        <tr>
                            <td>收货人：</td>
                            <td><input name="username" type="text"  value="${user.username}" id="username" required/></td>
                        </tr>
<%--                        <tr>--%>
...
<%--                        </tr>--%>
                        <tr>
                            <td>收货地址：</td>
                            <td><input name="fulladdress" type="text" value="${user.fulladdress}" id="fulladdress"  required/></td>
                        </tr>
                        <input name="sum" type="hidden" id="sum" value="${sum}"/>
                    </table>
                    <div class="FinishCart-bottom">
                        <input type="submit" name="Submit" value="提交订单">
                        <a class="FinishCart-return-cart" target="_self" role="button" href="${pageContext.request.contextPath}/toCart">返回购物车</a>
                    </div>
                </form>
